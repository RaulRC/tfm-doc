...
\begin{figure}[!h]
  \begin{center}
    \includegraphics[width=0.6\textwidth]{cdu-general2.png} 
...
\textit{Sea $G$ un grafo formado por un conjunto de nodos $N$ y un conjunto de
...
\begin{equation}
  U_n = \{<o,d_i,a_i> \in A_n \to isURI(a_i) \}
  \label{eq:metricI2}
\end{equation}
...
\textit{Sea $G$ un grafo formado por un conjunto de nodos $N$ y un conjunto de
  propiedades para los arcos $A$ de $G$. Sea $PD$ un conjunto de propiedades deseables. El valor de la
...
La medida vendrá determinada por las ecuaciones \ref{eq:metricSC1} y \ref{eq:metricSC2}:
